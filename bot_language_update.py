...
Reply ONLY based on the page and post information above. If the comment asks about anything not mentioned in the information, give a brief polite fallback response. Keep reply under 50 words. Match the language of the comment."""
        ...
        try:
            ...
            keywords_for_contact = ['how to order', 'buy', 'price', 'দাম', 'কিভাবে পাবো', 'কোথায় পাবো', 'যোগাযোগ',
            ...
            if should_add_contact:
                contact_string_parts = []
                if website_link and website_link not in reply:
                    contact_string_parts.append(f"ওয়েবসাইট: {website_link}")
                if whatsapp_number and whatsapp_number not in reply:
                    contact_string_parts.append(f"হোয়াটসঅ্যাপ: {whatsapp_number}")
                if facebook_group_link and facebook_group_link not in reply:
                    contact_string_parts.append(f"ফেইসবুক গ্রুপ: {facebook_group_link}")

                if contact_string_parts:
                    if comment_language == "bangla":
                        contact_prompt = "বিস্তারিত জানতে বা অর্ডার করতে যোগাযোগ করুন: " + ", ".join(
                            contact_string_parts) + "।"
                    else:
                        contact_prompt = "For details or to order, please contact us: " + ", ".join(
                            contact_string_parts) + "."

                    # Append contact info if it's not already there and the reply is not already too long
                    if len((reply + " " + contact_prompt).split()) <= 50:
                        reply = f"{reply.strip()} {contact_prompt.strip()}"
                    else:  # If adding makes it too long, prioritize contact info
                        if contact_string_parts:  # Only if contact info exists
                            if comment_language == "bangla":
                                reply = "বিস্তারিত জানতে বা অর্ডার করতে যোগাযোগ করুন: " + ", ".join(
                                    contact_string_parts) + "।"
                            else:
                                reply = "For details or to order, please contact us: " + ", ".join(
                                    contact_string_parts) + "."
                        else:  # Fallback to a generic short response if no contact info could be added
                            if comment_language == "bangla":
                                reply = "বিস্তারিত জানতে অনুগ্রহ করে ইনবক্স করুন।"
                            else:
                                reply = "Please inbox us for details."
            ...
            fallback_tokens = self.count_tokens(fallback_reply)
            return {
                "reply": fallback_reply,  # Use fallback reply on API error
                "sentiment": sentiment,
                "response_time": f"{response_time:.2f}s",
                ...
                "page_name": page_info.get("page_name", ""),
                "post_id": post_id,
                "output_tokens": fallback_tokens,  # Token count for fallback
                "note": f"API error: {e}. Fallback response used."
            }
...
# Flask route to handle incoming comment data
